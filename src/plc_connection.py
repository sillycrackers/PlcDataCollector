...
class PlcConnection:
    ...
    def _read_tags_with_trigger(self):
        ...
        with logix.PLC(timeout=3) as comm:
            ...
            elif trigger_response.Value == 1 and self.last_plc_trigger == 0:
                # Timestamp
    ...
    def _read_tags_at_specific_time(self):

        #Compare current hour and minute with saved specific time, if equal flag trigger
        if datetime.now().hour == self.plc.specific_time.hour and datetime.now().minute == self.plc.specific_time.minute:
            self.specific_time_trigger = True
        else:
            self.specific_time_trigger = False

        #Only collect data if specific time is equal, and we only collected data once using "last trigger" as Oneshot
        if self.specific_time_trigger and self.specific_time_last_trigger == False:
    ...
    def collect_data(self):
        ...
        if self.plc.trigger_type == TriggerType.PLC_TRIGGER:
            data = self._read_tags_with_trigger()
        elif self.plc.trigger_type == TriggerType.TIME:
